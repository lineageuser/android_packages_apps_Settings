...
public class ConnectivityCheckPreferenceController extends BasePreferenceController
    ...
    private void setCaptivePortalURLs(int mode) {
        ...
        String https = GRAPHENEOS_PORTAL.get(HTTPS_INDEX);
        String http = GRAPHENEOS_PORTAL.get(HTTP_INDEX);
        String fallback = GRAPHENEOS_PORTAL.get(FALLBACK_INDEX);
        String anotherFallback = GRAPHENEOS_PORTAL.get(OTHER_FALLBACK_INDEX);
        if (enabled) {
